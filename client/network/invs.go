...
func (c *OneConnection) ProcessInv(pl []byte) {
	if len(pl) < 37 {
		logg.Debug(c.PeerAddr.IP(), "inv payload too short", len(pl))
		c.DoS("InvEmpty")
		return
	}
	c.Mutex.Lock()
			...
			if !ahr {
				...
				if !blockReceived(bhash) {
					...
					if b2g, ok := BlocksToGet[bhash.BIdx()]; ok {
						...
						logg.Debug(c.PeerAddr.IP(), c.Node.Version, "also knows the block", b2g.Block.Height, bhash.String())
						c.MutexSetBool(&c.X.GetBlocksDataNow, true)
					} else {
						common.CountSafe("InvBlockNew")
						c.ReceiveHeadersNow()
						logg.Debug(c.PeerAddr.IP(), c.Node.Version, "possibly new block", bhash.String())
					}
					MutexRcv.Unlock()
				} else {
...
func NetRouteInv(typ uint32, h *btc.Uint256, fromConn *OneConnection) uint32 {
	...
	if typ == MsgTx {
		...
		if tx, ok := TransactionsToSend[h.BIdx()]; ok {
			...
			logg.Debug("NetRouteInv: txid", h.String(), "not in mempool")
		}
		TxMutex.Unlock()
	}
...
func (c *OneConnection) GetBlocks(pl []byte) {
	...
	if e != nil || len(h2get) < 1 || hashstop == nil {
		logg.Debug("GetBlocks: error parsing payload from", c.PeerAddr.IP())
		c.DoS("BadGetBlks")
		return
	}
...
func (c *OneConnection) SendInvs() (res bool) {
	...
	if bBlk.Len() > 0 {
		...
		logg.Debug("sent block's header(s)", bBlk.Len(), uint64(bBlk.Len()/81))
	}
